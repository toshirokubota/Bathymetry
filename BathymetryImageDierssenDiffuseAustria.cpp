...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	int startX = max(0, inarg.Get("-x0", 0));
	int endX = min(image.GetWidth(), inarg.Get("-x1", image.GetWidth()));
	int startY = max(0, inarg.Get("-y0", 0));
	int endY = min(image.GetHeight(), inarg.Get("-y1", image.GetHeight()));
	...
	out << "Start@\t" << startX << "\t" << startY << endl;
	out << "End@\t" << endX << "\t" << endY << endl;
	out << "X" << "\t" << "Y" << "\t";
	for(int i=0; i<method.NumEstimates(); ++i)
	{
	...
	int numFailed = 0;
	for(int i=startY, k=0; i<endY; ++i)
	{
		for(int j=startX; j<endX; ++j, ++k)
		{
			real x = (real)j; // param.GetParameter("X", k);
			real y = (real)i; // param.GetParameter("Y", k);
